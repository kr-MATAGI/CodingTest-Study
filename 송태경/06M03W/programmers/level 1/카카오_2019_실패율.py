...
def solution(N, stages):
    key_list = [i for i in range(1, N+1)]
    value_list = []
    st_len = len(stages)
    ...
    dd_dict = {}
    for i in range(N):
        dd_dict[key_list[i]] = value_list[i]

    aaa = sorted(dd_dict.items(), key = lambda item: item[1], reverse=True)
    answer = []
    for i in aaa:
        answer.append(i[0])
...
def solution(N, stages):
  ...
  keys는 생략이 가능합니다. 거기에 lambda는 기준을 result[x]: 즉 value로 정렬한다는 뜻입니다. 
  그래서 key가 출력되게 됩니다.
  """
