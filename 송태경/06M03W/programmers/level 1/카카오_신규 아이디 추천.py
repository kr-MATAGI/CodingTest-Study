...
def remove_sign(new_id):
    can_use = [chr(i) for i in range(ord('a'), ord('z')+1)] 
    can_use += ["-", "_", "."]
    can_use += [str(i) for i in range(0,10)]
    temp = []
    for i in new_id:
        if i in can_use:
            temp.append(i)
...
def remove_redundant_point(new_id):
    new_id=list(new_id)
    sp = []
    if len(new_id) == 1:
        if new_id[0] == '.':
...
def no_zero_len(new_id):
    if len(new_id) == 0:
        return "a"
    else: 
        return new_id

def no_over_15(new_id):   
    new_id = list(new_id)
    while(True):
        if len(new_id) >= 16:
            del new_id[len(new_id)-1]
...
def solution(new_id):
    new_id = new_id.lower()
    new_id = remove_sign(new_id)
    new_id = remove_redundant_point(new_id)
    new_id = remove_se_dot(new_id)
    new_id = no_zero_len(new_id)
    new_id = no_over_15(new_id)
    new_id = len_to_3(new_id)
    new_id = remove_se_dot(new_id)
    return new_id
...
print(re_solution("...!@BaT#*..y.abcdefghijklm"))